				...
				<div class="bc-chat-content-message" v-for="message in messages" v-if="message.type == 'answer'">
				    ...
				    <div style="background:#eee;padding: 20px">
				        <Card :bordered="false">
				            ...
				            <p v-html="message.content" v-if="!isRecommend || message.content == '你好'"></p>
				            <p v-else v-for="recommends in recommendList" @click="sendRecommend">{{ recommends.questionContent }}</p>
				        </Card>
				    </div>
					</template>
				</div>
...
import ipAddress from './config.js'
	export default {
	    ...
	    .then((res) => {
	      this.messages.push({ 'type': 'answer','content': res.body.robotInfo.helloReply})
    ...
    methods: {
			sendMessage () {
				...
				this.isLoad = true;
				this.$http({
            url: `${ipAddress.ip}/DeltaRobot/business/reply.action?userQuestion=${this.myText}&userId=1`,
            method: 'POST',
            // 请求体重发送的数据
            data: {
                // userQuestion: this.myText,
                // userId: 1
            },
            // 设置请求头
            headers: {
		    ...
		    .then((res) => {
		    	...
		    	console.log(res);
		    	this.isLoad = false;
		    	if (res.body.businessType == 1 || res.body.businessType == 3) {
		    		this.messages.push({ 'type': 'question','content': this.myText});
		      	this.messages.push({ 'type': 'answer','content': res.body.answer});
		      	...
		      	this.isRecommend = false;
		    		...
		    		console.log(res.body.recommend);
						this.messages.push({ 'type': 'question','content': this.myText});
						res.body.recommend.forEach((item) => {
							this.questionContent += item.questionContent;
						  //this.messages.push({ 'type': 'answer','content': item.questionContent})
						})
						if (this.questionContent) {
							this.messages.push({ 'type': 'answer','content': this.questionContent});
						}

		      	this.recommendList= res.body.recommend;
		    	}
			...
			sendRecommend () {
				alert("success")
				this.myText = arguments[0].srcElement.innerText;
				this.sendMessage();
			}
    }
  }
