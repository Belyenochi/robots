...
import ipAddress from './config.js'
    ...
    methods: {
			sendMessage () {
				...
				this.isLoad = true;
				this.$http({
            url: 'http://localhost:3000/greet.action?msg=' + this.myText,
            method: 'get',
            // 请求体重发送的数据
            data: {
                // data
		    ...
		    .then((res) => {
		      ...
		      this.$http({
            url: `${ipAddress.ip}/DeltaRobot/greet/record.action?greetContent=${this.myText}&greetAnswer=${res.body.content}&userId=1`,
            method: 'POST',
            // 请求体重发送的数据
            data: {
                // data
